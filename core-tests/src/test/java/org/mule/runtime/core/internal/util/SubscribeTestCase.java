...
public class SubscribeTestCase extends AbstractMuleTestCase {
  ...
  public void testSubscription() {
    ...
    Flux<String> upstream = Flux.<String>create(sink -> {
      sink.next("Hello");
      sink.complete();
    });
    ...
    Flux<String> combined = from(propagateCompletion(upstream, getDownstream(downStream, complete),
                                                     pub -> from(pub)
  ...
  private static Flux<String> getDownstream(FluxSinkRecorder<String> downStream, AtomicBoolean complete) {
    return downStream.flux()
